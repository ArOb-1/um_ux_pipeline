...
from app.utils import Console, ReportGenerator, logger, Colors
...
async def async_main(args):
    ...
    if args.show:
        ...
        if by_product:
            ...
            sorted_products = sorted(by_product, key=lambda x: x.get('avg_umux', 0), reverse=True)
            for p in sorted_products:
                avg = p.get('avg_umux', 0)
                color = Colors.GREEN if avg >= 80 else Colors.YELLOW if avg >= 60 else Colors.RED
                print(f"  • {p.get('product', '')}: {color}{avg:.2f}{Colors.ENDC} ({p.get('count', 0)} ответов)")
        return
    ...
    try:
        ...
        Console.success("Отчет: results/report.md")

        dashboard_path = Path("results/dashboard.html")
        if dashboard_path.exists():
            Console.success('Дашборд: results/dashboard.html')
        else:
            Console.warning('Дашборд не создан')

        logger.info(f'Пайплайн завершен. Валидных: {result.total_valid},'
                    f' Отбраковано: {result.total_rejected},'
